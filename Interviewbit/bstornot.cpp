...
bool bstornot(struct node* root,int minl,int maxl){
	if(root==NULL)
	return 1;
	if(root->data>minl && root->data<maxl){
		return bstornot(root->left,minl,root->data)&&bstornot(root->right,root->data,maxl);
		return 0;
	}
	return 0;
}
  ...
  while(1)
 {
    ...
    switch(d)
    {
        ...
        case 3:
                ...
                break;
        case 4: 
        		minl=INT_MIN,maxl=INT_MAX;
        		if(bstornot(root,minl,maxl))
